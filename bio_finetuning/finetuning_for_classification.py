...
class InstructionDataset(Dataset):
    def __init__(self, data_path, model_path, max_words=30):
    ...
    def __getitem__(self, index):
        ...
        if ann.get("input", "") == "":
            ...
            ann.update({"input": ann["input"][:350]})
...
def get_args_parser():
    ...
    parser.add_argument("--training_mode", default="masked_lm")
    parser.add_argument("--num_gos", default=0, type=int)
    return parser
...
def main(args):
    ...
    if args.training_mode == "masked_lm":
        dataset_train = InstructionDataset(
            data_path=train_data_path, model_path=args.llama_model_path, max_words=args.max_seq_len)
        dataset_val = InstructionDataset(
            data_path=valid_data_path, model_path=args.llama_model_path, max_words=args.max_seq_len
        )
    elif args.training_mode == "classification":
        go_terms_file = os.path.join(args.data_path, "terms.pkl")
        terms_df = pd.read_pickle(go_terms_file)
        terms = terms_df['gos'].values.flatten()
        terms_dict = {v: i for i, v in enumerate(terms)}

        dataset_train = InstructionDatasetForClassification(
            data_path=train_data_path, model_path=args.llama_model_path, max_words=args.max_seq_len, terms_dict = terms_dict
        )
        dataset_val = InstructionDatasetForClassification(
            data_path=valid_data_path, model_path=args.llama_model_path, max_words=args.max_seq_len, terms_dict = terms_dict
        )
        args.num_gos = len(terms_dict)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, data_loader_train, optimizer, device, epoch, loss_scaler, args.training_mode, log_writer=log_writer, args=args
        )

        val_stats = val_one_epoch(
            model, data_loader_val, optimizer, device, epoch, loss_scaler, args.training_mode, log_writer=log_writer, args=args
        )
